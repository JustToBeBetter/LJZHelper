...
+(void)showAdvertiserView:(NSArray *)imageArray
{
    ...
    NSString *filePath = [[AdHelper sharedInstance] getFilePathWithImageName:[kAdUserDefaults valueForKey:adImageName]];
    
    BOOL isExist = [[AdHelper sharedInstance] isFileExistWithFilePath:filePath];
    if (isExist) {// 图片存在
        
        AdView *adView = [[AdView alloc] initWithFrame:kAdMain_Screen_Bounds];
    ...
    [[AdHelper sharedInstance] getAdvertisingImage:imageArray];
}
